  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 gap-8 p-8 sm:p-12 md:grid-cols-[1fr_1fr] md:items-center md:gap-4">
        ...
        <IsoStack layers={layers} />
      </div>
    </section>
  )
}

function IsoStack({ layers }: { layers: CtaAppStackLayer[] }) {
  return (
    <div
      aria-hidden
        ...
        {layers.map((layer, i) => {
          ...
          return (
            <div
              key={i}
              className={cn(
                "absolute top-1/2 left-1/2 flex size-36 -translate-x-1/2 -translate-y-1/2 items-center justify-center overflow-hidden rounded-[22%]",
                "shadow-[0_30px_60px_-15px_rgba(0,0,0,0.6)]",
                layer.bg
              )}
              style={{
                transform: `translateZ(${fromBottom * 30}px)`,
              }}
            >
              {layer.content}
            </div>
